...
void question7() {
    int building_num = 0;
    cin >> building_num;
    vector<int> build_heigh_list(building_num, 0);
    for (int i = 0; i < building_num; i++) {
        cin >> build_heigh_list[i];
    }
    // energy left after a build is 2E - H
    double energy = 0.0;
    for (int i = building_num - 1; i >= 0; i--) {
        energy = (energy + build_heigh_list[i]) / 2.0;
    }
    cout <<  ceil(energy) << std::endl;
}
